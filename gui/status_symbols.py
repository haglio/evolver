"""One coloured symbol per status, shared by every view that shows a status.
...
marks — a run's green check is its stages' green check — and the colour goes on
the symbol alone. A failed run is a red ✘ beside a plainly-coloured timestamp,
...
def mark_for(status: str) -> tuple[str, QColor]:
    """The glyph and colour that stand for *status*.
...
def mark_icon(status: str, size: int = 16) -> QIcon:
    """*status*'s mark as an icon, for a view that cannot colour part of a row.

    A list item's text takes one colour for the whole string, so the run history
    could only redden the ✘ by reddening the timestamp beside it. Carrying the
    mark as the item's icon puts the colour back on the symbol, and drawing that
