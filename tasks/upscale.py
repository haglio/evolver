...
def run(priority_files: list[Path] | None = None, max_items: int | None = None) -> UpscaleResult:
    ...
    log.info("=== Stage 6: upscale from 1_sorted ===")
    ...
    for in_file, source, orient in candidates:
        ...
        if run_budget_seconds and (result.processed or result.failed) and remaining_budget < min_start_remaining_seconds:
            log.info(
                "Stopping Stage 6 before starting another video to stay under the run budget "
        ...
        if _is_low_disk():
            ...
            log.warning("Stopping Stage 6 early due to low free disk space.")
            break
...
def _show_low_disk_warning() -> None:
    ...
    show_error_window(
        ...
        (
            "Evolver paused Stage 6 because free disk space is below the configured safety floor.\n\n"
