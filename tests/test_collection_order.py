...
# This file, whose tests are plain module-level functions: pytest collects those
# in source order, while it hands a `unittest.TestCase`'s methods over in
# alphabetical order -- and every other file in this suite is TestCase-based
# (backlog item 11 is the conversion). Comparing against a file whose collected
# order is not its written order would be comparing the wrong two things.
