...
class TestNonAiUpscaleSummary(unittest.TestCase):
    ...
    def test_a_died_encode_names_what_failed(self):
        ...
        summary = self._result(failed="winston/1 clips/redacted Overload 1.mp4",
                               start_deferred="cooldown", pending=399)
        self.assertIn("failed", summary)
        self.assertIn("winston/1 clips/redacted Overload 1.mp4", summary)

    def test_a_fresh_start_names_the_video_it_kicked_off(self):
        summary = self._result(started="winston/1 clips/redacted It Dry 9.mp4")
        self.assertIn("started", summary)
        self.assertIn("winston/1 clips/redacted It Dry 9.mp4", summary)
    ...
    def test_a_stopped_encode_says_the_clip_keeps_its_place(self):
        ...
        summary = self._result(stopped="winston/1 clips/redacted redacted 4.mp4")
        self.assertIn("stopped", summary)
        self.assertIn("winston/1 clips/redacted redacted 4.mp4", summary)
