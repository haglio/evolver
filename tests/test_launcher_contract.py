...
@on_windows
def test_a_preview_runs_this_worktree_on_the_primary_venv_with_the_primarys_overlay():
    """The overlay is where library_root and project_roots live, so a copy taken
    weeks ago resolves a library that has moved; it is brought across every time."""
    ...
    assert Path(report.value("directory")) == REPO_ROOT
    assert report.value("arguments") == "preview_branch.py"
