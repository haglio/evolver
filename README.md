...
3. **Taken Back** — deletes every copy this library holds of a clip Origenerator has taken back. A send is a file copy into `0_inbox/<source>/` and nothing more, so undoing one is a delete here — and by the time it is asked for the clip has been sorted, renamed by the upscale and, down the Genau lane, moved into `videos/genau/clips/`, none of which Origenerator knows about. It withdraws the clip on its own gallery row instead, and this stage reads that the same read-only way the metadata scrape reads prompts. The withdrawal is a standing answer rather than a request that drains: nothing here can tell Origenerator the delete has happened, so the row goes on saying the clip does not belong and this stage goes on finding nothing. It runs before the scrape and the sort, so a clip on its way out is neither given a sidecar nor filed into `1_sorted` from an inbox it is about to leave.
4. **Metadata Scrape** — scrapes prompt metadata for AI videos in `1_sorted` into `videos/metadata`, mirroring the outbox tree. The scan is idempotent: a video that already has a metadata JSON is skipped, and one whose scrape fails is marked so it is not retried every run.
5. **Sort Inbox** — moves videos from `0_inbox/<source>/` into `1_sorted/<source>/<orientation>/`.
6. **Upscale** — upscales and interpolates sorted videos with Topaz Video AI ffmpeg. Work is capped per scheduler run and newly sorted inbox files go first. It holds off, with the reason `topaz_sign_in_expired`, while Topaz Video's sign-in has expired (see "Topaz sign-in" below).
7. **Genau Delivery** — moves the Genau lane's upscaled loops — Origenerator's looping single-cycle clips, which arrive under their own `0_inbox` source folder, named by the content overlay — out of the outbox into `videos/genau/clips/`, the one folder Genau plays from, retiring the `1_sorted` copy along with them. Those clips come through the pipeline only to be upscaled: a loop straight out of the graph is visibly softer than the clips already in that folder, which came from upscaled library video. Both halves have to leave together — the upscale stage decides what still needs doing by looking for the output beside its source, and the correspondence check requires each sorted video to have a `_topaz` counterpart.
8. **Upscale non-AI** — gradually upscales the `2D/non_AI` library too, with the recipe its already-processed clips carry in their `videoai` tags (see "Non-AI library upscaling" below). Off by default — a one-time opt-in from the tray menu, after which Evolver runs at most one detached encode while you are idle and the AI queue is drained, suspending it the moment you return to the machine.
9. **Correspondence Check** — verifies `1_sorted` and `2_outbox` are in 1-to-1 correspondence, each sorted file having an outbox counterpart named `<sorted_stem>_topaz<ext>`, with a Windows error dialog if mismatches remain.
10. **Follow Moved Videos** — repoints the suite's saved video paths at videos that have since moved, so a Clipper session or a Fun Time favorite survives the library being rearranged (see "Following videos that moved" below).
11. **Watch Weights** — sums what Fun Time and Warm Gun each completed, skipped and locked onto every library video's sidecar, as a `watch` block carrying the playback weight both apps shuffle by; applies the phone's favorites and unfavorites to `fun_time/favs.csv`; and flags every favorite on its sidecar. See "What both apps watched" below.
12. **Bookmarks Sync** — prunes stale rows from `fun_time/favs.csv` whose local file is gone, then syncs the remaining `web_url` values into a `Fun Time Favs` folder on the Chrome bookmarks bar. It runs after the repointing above, so a row is dropped only when its video is genuinely gone rather than merely moved.
13. **Clip Scripts** — cuts each carved clip's funscript out of its source scene's, using the offset the clip was matched at.
14. **Scene Scripts** — gives an unscripted source scene a mostly-blank funscript holding its carved clip's, placed where the clip sits in it.
15. **Scripts Sync** — rehomes `.funscript` files under `videos/scripts/scripts` so they mirror the matched video path under `videos/videos`. A script only moves when there is exactly one basename match in the same library lane; scripts under `2D/AI` only consider `2D/AI` videos, and scripts under `2D/non_AI` only consider `2D/non_AI` videos. A script whose video is gone from the library but present in the `retired_root` archive (see "Non-AI library upscaling" below) follows it out and comes to sit beside it; a stranded duplicate of a script the archive already holds is discarded once the two compare byte-identical. A script that matches no video at all moves to `videos/scripts/unmatched_scripts`, and the popup names it with a link to that folder; renamed there to match its video, it moves into place on the next run, and while it waits there it brings no popup back. A script that matches more than one video is left where it is and named in the popup. VR videos kept off the library's drive (`vr_videos` in the content overlay) are matched as though they sat in `videos/videos/VR`. It also copies missing funscripts across matching processed/original video variants.
16. **Group non-AI** — records each `2D/non_AI` clip's version family, the original and its processed variants, in a mirrored metadata sidecar. A copy saved under a name of its own joins the family too when it runs the same length and shows the same pictures: a video with a same-length neighbor in its bucket has its picture read at 16 evenly spaced moments, once, kept on its sidecar, and two that agree at most of those moments are one video. `version_overrides` in the content overlay is left for the pairs the pictures cannot settle, such as a trim saved under a new name.
17. **Title non-AI** — records what to call each `2D/non_AI` video: the performer and movie its clip record names, carried across to the whole scene that clip was cut out of, so every app reads one name rather than working it out at launch. A video no clip record names keeps none, and its readers fall back to the filename.
18. **Video Kinds** — records what kind each library video is — a generation, an excerpt carved from a longer scene, a Genau clip — and how long it runs, on its mirrored metadata sidecar, so the apps that ask can read both rather than measure them.
19. **Provenance** — records on each library video's metadata sidecar what made it: which app, at which version of its code, under which recipe. The stages that make something stamp it as they do — the upscales, the Origenerator scrape, the two script stages (inside the funscript itself) — so this one fills in only what they could not: a video made before any of that was recorded, or one that arrived by a path that stamps nothing, such as an upscale exported by hand from the Topaz GUI. What made an Origenerator clip is looked up in Origenerator's own gallery, and an upscale's recipe is read from the note Topaz wrote into the file — a few hundred files a run, until the whole library has been read once. Whatever neither can say, the version of a recipe above all, is recorded as unknown rather than guessed, so a later sweep that remakes old work never skips a file believing it current.
20. **Duplicate Check** — scans the `non_AI` folder for likely accidental duplicates: video files with the same exact filesize but different filenames, with a Windows error dialog if any are found.
...
12. Records what kind every video in the library is — `genau_clip`, `excerpt`, `short` or `full_length` — on its metadata sidecar, as `video.type`. One field, written here and read by Fun Time, the main player and Warm Gun in place of the several tests each of them used to run (a running time against a threshold each picked separately, the folder the Genau loops are delivered to, the presence of a `clip` record, and "everything else"). The stage walks the whole library every run, so it backfills a library that has never been asked and keeps up with the ones arriving, out of the same code and with no one-off script to remember. What it asks depends on what the answer costs: where a video sits and whether its sidecar says something carved it out of a longer video are free to read, so those two kinds are re-derived every run and the record corrects itself — declare a folder of excerpts in the overlay, or split a video and write it a `clip` record, and the next run fixes what it wrote before it knew. A running time costs an ffprobe, so that answer is remembered once — written down beside the kind as `video.duration_seconds`, for this stage and for anything else that wants it — and a run measures at most `VIDEO_TYPE_BATCH_LIMIT` videos, so the first passes over a big library spread over a few runs instead of holding one up, and a video ffprobe cannot read is left for the next run rather than guessed at. Every video is measured, including the ones whose kind was free: an excerpt is an excerpt however long it runs, but its length is what the non-AI upscale stage weighs its queue by, and two thirds of the videos that stage accounts for are excerpts. A free kind still lands when the measurement fails — the kind never waits on a running time. Excerpts — scenes carved out of longer videos — are known by the `clip` record on their sidecar; for the batches split before anything wrote one, by the company they keep, since a batch whose librarian filed its cuts into a folder of their own put nothing else in there; and failing both, by the folders `excerpt_folders` names in the content overlay.
...
- Modules:
  ...
  - `tasks/nonai_progress.py` - how far through the non-AI upscale project the library is, by running time
  - `tasks/nonai_encode.py` - the detached encode itself: start, adopt, freeze, measure, kill
  - `tasks/nonai_lineup.py` - that queue as the queue window shows it: one list, with the video in flight on top
  - `util/upscale_lineup.py` - the shape that window is handed, and the five words for what the video in flight is doing
  - `util/nonai_job.py` - the four JSON files that encode keeps its state in
  ...
  - `util/video_type.py` - the four kinds a video can be, which one a video is, and how long it runs
  - `tasks/video_types.py` - recording both on every library video's sidecar
...
Say an act, always prefixed with a camera word. The camera words are content, like the acts, so they come from the content overlay's `cameras` key; the committed example ships `side` and a placeholder initialism, `XYZ`. Every clip is tagged with one of them; no act has a bare, camera-less form:
...
So "side beta" records `Side Beta`, and "X-Y-Z zeta" records `XYZ Zeta` — matching the `XYZ Gamma` form the prompt scraper writes, which cases a camera word the way the overlay prefixes it, so one Fun Time filter query reaches both. Even `dance` and `other` take a camera word: "side dance" records `Side Dancing`. A camera word that is an initialism lists its spelled-out letters as an alias (`x y z ...`), and that is the form the recognizer catches, because the vosk lexicon prices each letter as its name while the one-word form is a guess at a single syllable; both are accepted, whichever way you happen to say it.
...
Topaz Video's ffmpeg runs on the sign-in the Topaz Video app saves, and that sign-in lasts 30 days. Once it has expired, Topaz renews it over the internet at the start of every encode instead, and when a renewal fails it puts a flickering watermark on everything that encode makes, while the encode otherwise succeeds. So both upscale stages ask Topaz first: a fraction-of-a-second test clip run at Topaz's detailed log level, which is the only place Topaz says whether the saved sign-in has expired. The question is asked only when an upscale could otherwise start, never with nothing to upscale and never beside a Topaz encode that is already running.

While the sign-in has expired, the AI upscale is skipped with the reason `topaz_sign_in_expired`, no non-AI encode starts (`deferred=topaz_sign_in_expired`), and the tray pops up a message asking you to open the Topaz Video app and sign in, at most once a day however many runs it holds back. Signing in is the whole fix: the next run finds the sign-in good and upscaling carries on.
...
**Order**: pinned videos go first, in the order they are listed in `.nonai-upscale-next.txt` (repo root, gitignored, same `path<TAB>note` format as the skip list — the counterpart to it: "encode this one next" rather than "never encode this"). The queue window below writes that file too, so the pins are either hand-written or dragged into place. A pin outranks every heuristic below it and also re-queues a video the bucket already holds an older processed variant of. Then an explicit `1 could use work` flag; then Fun Time watch score, descending — read straight from `fun_time/state/watch_stats.json` with the same `completions + 3×locks − skips` arithmetic its playlist breeding uses, so once Fun Time starts tracking primary (main player) plays, the most-watched videos move to the front on their own; funscript ownership breaks ties among the unwatched (a fair proxy, since the main player drives the OSR2 with them); then everything else alphabetically.
...
**Gradually** means: one video at a time — and more broadly, **one Topaz process working on the machine at a time**, because stacked Topaz encodes are what exhausted memory and crashed the machine. The stage refuses to start while *any* Topaz ffmpeg is running (its own, an orphan, or a manual GUI export). The AI upscale stage waits (`skip_reason: topaz_busy`) only for a Topaz process Evolver did not start: the non-AI encode it did start is frozen while the AI clips upscale, a minute or so each, and carries on where it stopped once they are done — so a clip sent to Genau or to Evolver never waits on a library video that can take days to finish while you are at the computer. These encodes take hours, and the tray watchdog kills a pipeline run at eleven minutes, so the stage never waits on ffmpeg: it launches a single detached, below-normal-priority Topaz process and returns; each later scheduler tick checks on it. A finished encode (output duration ≥ 98% of the source's) is promoted and its original retired; an interrupted or failed one is retried once and then parked in `.nonai-upscale-skip.txt` (repo root, gitignored, one `path<TAB>reason` per line — delete a line to retry it). An encode still *actively* running after 24 hours is killed, but only after confirming the pid still belongs to Topaz ffmpeg — time spent suspended for user presence is banked in the job and subtracted, so a heavily-paused encode is never mistaken for a stuck one.

A new encode starts only when *all* of these hold: the toggle is on, the user has been idle past the presence threshold, the AI upscale queue is drained, no Topaz ffmpeg is already running, CPU is below the busy threshold, at least `NONAI_MIN_AVAILABLE_RAM_GB` (8) of RAM is free, free disk is above the safety floor, and at least `NONAI_COOLDOWN_MINUTES` (30) have passed since the previous encode ended — so an unattended night alternates encode / breather instead of running the GPU flat out end to end — and Topaz Video's sign-in has not expired (see "Topaz sign-in" below). The tick log's `deferred=` field says which gate held a start back (`user_present`, `topaz_busy`, `low_ram`, `ai_clips_waiting`, `cooldown`, `topaz_sign_in_expired`). A video *asked for* from the queue window answers to fewer of them: it starts whatever the toggle says, whoever is at the computer and however recently the last encode ended, and is held back only by the machine's own limits — another Topaz process, low memory, a nearly full drive, an expired sign-in — and by newly downloaded AI clips, which take a minute each and cannot run beside it. The disk floor is also enforced *during* an encode: if free space crosses it mid-write, the encode is stopped (no retry penalty) instead of running toward a full disk.
...
**How far along it is** is weighed by running time, not by clip count, because the two say different things: the day it was written the library was 59% upscaled by clip and 29% by running time, the short clips having gone first. The project is what the queue holds plus what the library already has an upscale of — however that upscale was made, since a good part of the library was done by hand in the Topaz GUI before this stage existed. A clip the project cannot reach is in neither total: one parked in a sub-stage that still wants a human with a trimmer, one the skip list retired, one sitting in a bucket's "good to go" that nobody asked to re-encode. The running times come off the sidecars the **Video Kinds** stage writes, so this costs a walk and a JSON read per video rather than four hundred ffprobe spawns every ten minutes; until that stage has been over the library, videos with no recorded time are counted apart (`… 12 not yet measured`) and left out of both totals rather than guessed at.
...
Fun Time counts what its players complete, skip and lock in `fun_time/state/watch_stats.json`. Warm Gun, on the phone, writes the same events — and `favorite`, `unfavorite`, `weird` — one JSON line each into `videos/warm_gun/*.jsonl`, a folder inside the library so the file sync that carries the videos carries the journal here too. While the app still writes to a folder of its own outside the library, name that folder as `warm_gun_outbox` in the content overlay and it is read as well — a line reached through both folders counts once, so the app can be changed and the folder retired without a day where the two sides disagree. A line is `{"t": <unix seconds>, "event": "...", "path": "..."}`, the path relative to the lane the phone played it from: `1_sorted/<source>/<orientation>/<file>`, `non_AI/<bucket>/...`, or `genau/clips/<file>`. Every journal in the folder is read and a line that appears in two of them counts once.
...
The main player steps from a clip to its scene by.
...
- Non-AI candidate discovery, priority order, and the detached-encode lifecycle: launch, in-flight, promote, retry, skip-list, and stuck-job kill (`tasks/nonai_upscale.py`)
