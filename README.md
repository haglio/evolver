...
15. **Video Kinds** — records what kind each library video is — a generation, an excerpt carved from a longer scene, a Genau clip — on its mirrored metadata sidecar, so the apps that ask can read it rather than measure it.
