...
5. **Upscale** — upscales and interpolates sorted videos with Topaz Video AI ffmpeg. Work is capped per scheduler run and newly sorted inbox files go first. It holds off, with the reason `topaz_sign_in_expired`, while Topaz Video's sign-in has expired (see "Topaz sign-in" below).
...
17. **Provenance** — records on each library video's metadata sidecar what made it: which app, at which version of its code, under which recipe. The stages that make something stamp it as they do — the upscales, the Origenerator scrape, the two script stages (inside the funscript itself) — so this one fills in only what they could not: a video made before any of that was recorded, or one that arrived by a path that stamps nothing, such as an upscale exported by hand from the Topaz GUI. What made an Origenerator clip is looked up in Origenerator's own gallery, and an upscale's recipe is read from the note Topaz wrote into the file — a few hundred files a run, until the whole library has been read once. Whatever neither can say, the version of a recipe above all, is recorded as unknown rather than guessed, so a later sweep that remakes old work never skips a file believing it current.
18. **Duplicate Check** — scans the `non_AI` folder for likely accidental duplicates: video files with the same exact filesize but different filenames, with a Windows error dialog if any are found.
