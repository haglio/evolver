...
A panel on the right lists every command as a clickable tile, laid out the way the vocabulary is: every act as a grid of `Side` / `POV` columns, then the controls. It is both the on-screen reference — no need to remember what the phrases are — and a fallback that never depends on the microphone: clicking a tile drives the exact same path a spoken phrase would, so a mishearing mic or a quiet room never leaves you unable to label. Each tile shows the action it records and names its spoken phrase on hover, and fills in with an example frame lifted from a clip the library already labels that act (matched case-insensitively, so an older `Pov Epsilon` clip still illustrates the `POV Epsilon` tile) — so the grid reads as a gallery you recognize at a glance. The frames are extracted once and cached under `config.BACKFILL_THUMBNAIL_DIR`; an act with no labelled example yet simply stays text-only. Building them runs in the background, so the window opens at once and tiles gain their pictures as they land.
