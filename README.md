...
**Watching it**: every tick appends one line to `evolver.log` — `Non-AI upscale: started=... in_flight=... promoted=... stopped=... failed=... pending=...` — so a `grep "Non-AI"` over the log is the quickest status. What's encoding right now (source, output, pid, start time) sits in `nonai_upscale_job.json`, the encode's own stderr streams to `nonai_upscale_ffmpeg.log`, and the growing `*.partial.*.mp4` in the bucket's `3*/processed/` folder shows byte-level progress. The same per-tick summary lands in each run's record, visible in the main window's run history under the **Upscale non-AI** stage row.
