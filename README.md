...
3. Rehomes `.funscript` files under `videos/scripts/scripts` so they mirror the matched video path under `videos/videos`. A script only moves when there is exactly one basename match in the video library; unmatched or ambiguous names are logged and left alone.
...
- Modules:
  ...
  - `tasks/scripts_sync.py` - Stage 3 funscript/video tree alignment
...
- Funscript alignment so `videos/scripts/scripts` mirrors `videos/videos` when basename matches are unique (`tasks/scripts_sync.py`)
...
- Stage 3 always runs after purge and only moves a script when its basename matches exactly one video in `videos/videos`.
