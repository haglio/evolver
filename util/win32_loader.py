...
the DLL handles, their argtypes and the COM prototypes are declared once, at
module scope, so the call sites stay bare.  That makes the binding a single
point of failure for far more than the code that needs Windows — off Windows
``ctypes`` has no ``windll`` at all, the import raises, and every test module
that reaches one of these modules is dropped from the run with it.  One import
then decides whether a suite exists.
